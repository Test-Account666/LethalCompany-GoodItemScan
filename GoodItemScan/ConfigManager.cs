...
public static class ConfigManager {
    ...
    internal static void Initialize(ConfigFile configFile) {
        preferClosestNodes = configFile.Bind("General", "Prefer Closest Nodes", true,
                                             "If true, will prefer scanning the closest nodes first. " + "This might cause performance issues.");

        alwaysRescan = configFile.Bind("General", "Always Rescan", true,
                                       "If true, will always start a fresh scan. " + "This removes all previously scanned nodes from the UI.");

        scanNodesHardLimit = configFile.Bind("General", "Scan Nodes Hard Limit", 120,
                                             new ConfigDescription(
                                                 "Defines the maximum amount of scan nodes on screen. "
                                               + "If you feel like your screen is cluttered, try lowering this value.", new AcceptableValueRange<int>(30, 666)));

        scanNodeDelay = configFile.Bind("General", "Scan Node Delay", 0.1F,
                                        new ConfigDescription(
                                            "Defines the delay between each scan node being added to the UI. " + "This will look stupid if set too high. "
                                          + "This value is divided by 100.", new AcceptableValueRange<float>(0, 1F)));

        useDictionaryCache = configFile.Bind("General", "Use Dictionary Cache", true,
                                             "May increase performance, at the cost of ram usage. " + "If true, will use a dictionary for caching. "
        ...
        maxScanNodesToProcessPerFrame = configFile.Bind("General", "Max Scan Nodes To Process Per Frame", 32,
                                                        new ConfigDescription(
                                                          ...
                                                          + "This value is used in updating and scanning nodes!", new AcceptableValueRange<int>(1, 666)));

        updateScanNodeText = configFile.Bind("General", "Update Scan Node Text", true,
                                             "If true, will update the scan node text. "
                                           + "This is useful, if you, for example, have a mod that updates the value of an item.");

        calculateDistance = configFile.Bind("General", "Calculate Distance", false,
                                            "If true, will perform additional distance calculations. " + "If false, use viewport as distance. "
                                                                                                       + "Set to `true`, if you don't mind slightly worse performance and want accurate distance calculations.");

        sendDebugMessages = configFile.Bind("Debug", "Send Debug Messages", false, "If set to true, will spam your log with debug messages.");
        ...
        totalAddWaitMultiplier = configFile.Bind("Special Cases", "Total Add Wait Multiplier", 100,
                                                 new ConfigDescription(
                                                   ...
                                                   + "The lower this number, the less will be added per updated." + "For a vanilla-ish feeling, set this to 48.",
        ...
        updateTimer = configFile.Bind("Special Cases", "Scan Node Update Timer", 1.2F,
                                      new ConfigDescription(
                                        ...
                                        + "The lower this number, the more often it will get updated.", new AcceptableValueRange<float>(0F, 12F)));

        fixLandmineScanNode = configFile.Bind("Special Cases", "Fix Landmine ScanNode", true, "Fixes Landmines being scannable after exploding.");
    }
}